...
export class RetweetService {
  ...
  async addRemoveRetweet(dto: any, user: User): Promise<any> {
    ...
    if (foundedRetweet) {
      ...
      if (foundedTweet.retweetsCount) {
        ...
        foundedTweet.retweets = foundedTweet.retweets.filter(
          (retweet) => retweet.id !== foundedRetweet.id,
        );
  ...
  async getAllRetweetedUsersOfTweet(
    tweetId: string,
    selfId: string,
    page: number = 1,
    pageSize: number = 10,
    ...
    let result: UsersList[] = await Promise.all(
      foundedRetweets.flatMap(async (u) => {
        if (u.user.id !== selfId) {
