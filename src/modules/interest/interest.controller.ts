...
import {
  ...
  successPaginatedResponseWithoutDB,
  successResponse,
} from '../../base/response';
...
export class InterestController {
  ...
  async getAll(
    ...
    try {
      const { interests, total } = await this.interestService.getAll(
        page,
        pageSize,
      );
  ...
  async createInterestByUser(
    ...
    try {
      let result = await this.interestService.createMultipleInterestsByUser(
        nameDto.interests,
        request.user,
      );
      successResponse(response, result);
  ...
  async createInterestByTweet(
    ...
    try {
      let result = await this.interestService.createMultipleInterestsByTweet(
        nameDto.interests,
        tweetId,
      );
      successResponse(response, result);
  ...
  async searchInterestByName(
    ...
    try {
      let { result, count } = await this.interestService.searchInterestByName(
        name,
  ...
  async searchInterestByNameWithUsers(
    ...
    try {
      let result = await this.interestService.searchInterestByNameWithUsers(
        name,
        (request.user as any).id,
      );
      successResponse(response, result);
  ...
  async searchInterestByNameWithTweets(
    ...
    try {
      let result = await this.interestService.searchInterestByNameWithTweets(
        name,
        (request.user as any).id,
      );
  ...
  async searchInterestByNameOnlyInterest(
    ...
    try {
      let { result, count } =
        await this.interestService.searchInterestByNameOnlyInterest(name);
      successPaginatedResponse(response, result, count, page, pageSize);
    } catch (error: any) {
      errorResponse(response, error.message);
    }
  ...
  async deleteMultipleInterestsByUser(
    ...
    try {
      let result = await this.interestService.deleteMultipleInterestsByUser(
        nameDto.interests,
        request.user,
      );
      successResponse(response, result);
