...
export class InterestService {
  ...
  async createMultipleInterestsByUser(
    names: string[],
    user: any,
  ): Promise<Interest[]> {
  ...
  async createMultipleInterestsByTweet(
    names: string[],
    tweetId: string,
  ): Promise<Interest[]> {
    const savedInterests: Interest[] = [];
  ...
  async searchInterestByName(
    ...
    let result: InterestsList[] = await Promise.all(
      interests.map(async (interest) => {
        const usersList = await Promise.all(
          interest.users.map(async (user) => {
            if (user.id !== selfId) {
          ...
          interest.users.map(async (user) => {
            if (user.id !== selfId) {
              return {
                ...
                user_isFollowing: await this.followService.isMyFollowing(
                  user.id,
                  selfId,
                ),
              };
