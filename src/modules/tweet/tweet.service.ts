...
export class TweetService {
  ...
  async create(dto: any, user: any, media?: any[]): Promise<any> {
    ...
    let newTweet: any = this.tweetRepository.create({
      media: tweetMedia,
      taggedUsers,
      ...dto,
    });
  ...
  async updateActionById(
    ...
    if (action === 'DELETE') {
      let userDetails = await this.userRepository.findOneBy({
        id: userId,
        deleteFlag: false,
      });
  ...
  async getTweetsByUserInterests(userId: string): Promise<TweetsList[]> {
    let user: any = await this.userRepository.findOne({
      ...
      relations: [
        'interests',
        'interests.tweets',
        'interests.tweets.user',
        'interests.tweets.interests',
        'interests.tweets.hashtags',
      ],
    });

    let interestTweetsList = await Promise.all(
      user.interests.map(async (interest: any) => {
        return await Promise.all(
          interest.tweets.map(async (tweet: any) => {
            return {
              id: tweet.id,
              text: tweet.text,
              media: tweet.media,
              ...
              isRetweeted: tweet.isRetweeted,
              isEdited: tweet.isEdited,
              selfLiked: await this.likeService.isTweetLikedByUser(
                tweet,
                userId,
              ),
              selfRetweeted: await this.retweetService.isTweetRetweetedByUser(
              ...
              selfBookmarked:
                await this.bookmarkService.isTweetBookmarkedByUser(
                  tweet,
                  userId,
                ),
              userId: tweet.user.id,
    ...
    return interestTweetsList.sort((a, b) => {
      return (
        new Date(b.tweet_createdAt).getTime() -
        new Date(a.tweet_createdAt).getTime()
      );
    });
  }

  async getTweetsOfFollowers(
    selfId: string,
    page: number = 1,
    pageSize: number = 10,
  ): Promise<any> {
