...
export class BookmarkService {
    ...
    async addRemoveBookmark(dto: any, user: User): Promise<any> {
        ...
        const foundedbookmark: Bookmark | null = await this.bookmarkRepository.findOne({
        ...
        if (foundedbookmark) {
            ...
            if (foundedTweet.bookmarksCount) {
                ...
                foundedTweet.bookmarks = foundedTweet.bookmarks.filter(bookmark => bookmark.id !== foundedbookmark.id);
    ...
    async getAllbookmarkedUsersOfTweet(tweetId: string, selfId: string, page: number = 1, pageSize: number = 10): Promise<any> {
        const foundedTweet: Tweet | null = await this.tweetRepository.findOne({ where: { id: tweetId } });
        if (!foundedTweet) throw new NotFoundException('Tweet not found');
        const [foundedbookmarks, count] = await this.bookmarkRepository.findAndCount({
    ...
    async isTweetBookmarkedByUser(tweet: Tweet, userId: string): Promise<boolean> {
