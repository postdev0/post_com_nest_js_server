import { Controller, Get, Post, Body, Param, Req, Res, UseGuards, UseInterceptors, UploadedFiles, Put, Query } from '@nestjs/common';
...
export class CommentController {
    ...
    async create(
        ...
        try {
            let result = await this.commentService.create(createCommentDto, request.user, tweetId, media);
    ...
    async update(
        ...
        try {
            let result = await this.commentService.update(id, updateCommentDto, media);
            successResponse(response, result);
        } catch (error: any) {
            errorResponse(response, error.message);
        }
    ...
    async findByTweet(
        ...
        try {
            let { result, count } = await this.commentService.getAllCommentsOfTweet(tweetId, page, pageSize);
