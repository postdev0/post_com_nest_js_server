...
export class LikeService {
    ...
    async addRemoveLike(dto: any, user: User): Promise<any> {
        ...
        if (foundedLike) {
            ...
            if (foundedTweet.likesCount) {
                ...
                foundedTweet.likes = foundedTweet.likes.filter(like => like.id !== foundedLike.id);
    ...
    async getAllLikedUsersOfTweet(tweetId: string, selfId: string, page: number = 1, pageSize: number = 10): Promise<any> {
