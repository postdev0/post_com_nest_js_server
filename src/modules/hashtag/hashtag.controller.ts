...
export class HashtagController {
  ...
  async getAll(
    ...
    try {
      const { hashtags, total } = await this.hashtagService.getAll(
        page,
        pageSize,
      );
  ...
  async createHashtagByTweet(
    ...
    try {
      let result = await this.hashtagService.createMultipleHashtagsByTweet(
        nameDto.hashtags,
        tweetId,
      );
  ...
  async searchInterestByName(
    ...
    try {
      let result = await this.hashtagService.searchHashtagByNameWithTweets(
        name,
  ...
  async searchInterestByNameWithTweets(
    ...
    try {
      let { result, count } = await this.hashtagService.searchHashtagByName(
        name,
        page,
        pageSize,
      );
  ...
  async getAllTweetsByHashtag(
    ...
    try {
      let result = await this.hashtagService.getAllTweetsByHashtag(
        name,
        (request.user as any).id,
      );
      successPaginatedResponseWithoutDB(response, result, page, pageSize);
